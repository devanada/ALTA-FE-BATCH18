...
function sortMerge(elements: number[]): number[] {
  console.time("SORT MERGE");
  const result = divide(elements);
  console.timeEnd("SORT MERGE");
  return result;
}

function divide(elements: number[]): number[] {
  if (elements.length < 2) return elements;

  let mid = Math.floor(elements.length / 2);
  let one = elements.slice(0, mid);
  let two = elements.slice(mid);
