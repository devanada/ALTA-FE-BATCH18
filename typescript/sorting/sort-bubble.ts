...
function sortBubble(elements: number[]): number[] {
  console.time("SORT BUBBLE");
  const len = elements.length;
  let dupe = elements.slice();

  for (let i = 0; i < len; i++) {
    for (let j = 0; j < len; j++) {
      // jika nilai pada dupe[j] lebih besar daripada nilai pada dupe[j + 1], maka posisinya ditukar
      if (dupe[j] > dupe[j + 1]) {
        let temp = dupe[j];
        dupe[j] = dupe[j + 1];
        dupe[j + 1] = temp;
