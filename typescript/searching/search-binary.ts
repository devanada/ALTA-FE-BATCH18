/*
Konsep binary search yaitu menggunakan array yang sudah tersortir secara berurutan. Pencarian dimulai dari element tengah, jika element tengah lebih kecil dari target maka nilai start geser ke kanan, jika element tengah lebih besar dari target maka nilai end geser ke kiri. Return -1 ketika target tidak ditemukan, return index ketika target ditemukan.
*/

function binarySearch(elements: number[], target: number): number {
...
const dupeArray = array.slice().sort((a, b) => a - b);

// console.log(binarySearch(array, 58)); // -1
console.log(binarySearch(dupeArray, 58)); // 57
